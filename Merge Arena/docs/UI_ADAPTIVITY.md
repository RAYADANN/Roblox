...
## Антипаттерн: несколько множителей и ручная подгонка

Адаптив часто **начинается правильно**, но разрастается:

| Проблема | Следствие |
|----------|-----------|
| Несколько множителей: `uiScale`, `textMult`, `chromeMult`, `dockMult` | Компоненты не знают, какую функцию вызывать |
| Размеры конкретных виджетов в ViewportLayout (`coinChipSize`, …) | Каждый новый чип = правка центрального модуля |
| Ручные override «на глаз» под phone/desktop | Дублирование, рассинхрон |
| `layoutEpoch` + subscribe в каждом компоненте | Шум вместо одного хука |

Итог: «под каждое устройство подгонять» — **симптом**, не норма.
