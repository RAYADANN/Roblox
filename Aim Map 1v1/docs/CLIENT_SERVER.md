...
| Тип | Триггер | Модуль |
|-----|---------|--------|
| **FP ViewModel** (руки+USP на камере) | Character spawn | `client/core/ViewModelController` |
| Screen flash, coin rain | `Notify` | `client/ui/fx/*FX.luau` |
| Combat screen wash (red hit / blue kill) | `TookDamage` / local `KillBannerState` | `client/ui/components/CombatFlashLayer` on `GameAim` |
| UI toast | ошибка Zap / Notify | `client/ui` + Notification |
| Звук клика | сразу на клиенте | `SoundManager` |

**ViewModel:** только LocalPlayer. Мировое тело + USP остаются на серверном персонаже (другие видят; себе скрыто через `LocalTransparencyModifier`). Анимации Idle/Shoot2 играются и на viewmodel, и на теле (репликация).

**USP fire:** клиент считает точку прицела лучом камеры через центр viewport, origin — Part `Shoot` на viewmodel, направление — `Shoot` → эта точка. Сервер валидирует origin (slack от головы) и делает hitscan. Локальный трассер сходится в impact; чужой трассер — из мирового `Shoot`. Дuel bot: то же самое с мирового `Shoot` (Idle на сервере, чтобы дуло совпадало с видимым хватом).
