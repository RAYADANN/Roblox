...
│   │   ├── OreDatabase.luau     # конфиги (данные, не код)
│   │   └── ...
│   └── util/
│       ├── RebirthLogic.luau    # pure, тестируется
│       ├── UpgradeLogic.luau
│       └── ...
│
├── server/
...
│   │   ├── MiningManager.luau   # фича-менеджер
│   │   ├── EconomyManager.luau
│   │   └── ...
...
└── client/
    ├── core/
    │   ├── MiningRenderer.luau  # мир, не React
    │   ├── SoundManager.luau
    │   └── ...
    ├── ui/
...
### Игровое действие (пример: копание)

```
Client: click block
  → Zap: MineBlock(blockId)          -- намерение
  → Server MiningManager:
      validate (reach, cooldown, CPS)
      MiningLogic.rollOre(depth, rng)
      mutate profile
      saveProfile
  → Zap: MineResult + Notify
  → Client: MiningRenderer + HUD update
```

**Клиент НЕ меняет coins/depth/inventory напрямую.**
...
3. `MiningManager` (deps: profile, economy, notify)
4. `QuestManager`, `PetManager`, …

### init.client.luau (≤80 строк)

```luau
-- 1. ViewportLayout.start()
-- 2. require контроллеры (MiningRenderer, SoundManager)
...
## Антипаттерны из прошлого проекта

| Было | Правильно |
|------|-----------|
| `init.server` 870 строк | Тонкий boot + менеджеры |
| `MiningRenderer` 1800 строк | Дробить на sub-renderers |
...
| Питомец только на LocalPlayer | Server publish → all clients render |
